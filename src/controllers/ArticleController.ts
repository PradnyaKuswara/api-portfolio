...
class ArticleController {
  async all(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const pageNumber: number | undefined = page
        ? parseInt(page as string, 10)
        : undefined;
      const limitNumber: number | undefined = limit
        ? parseInt(limit as string, 10)
        : undefined;

      const [articles, totalArticles]: [Article[], number] =
  ...
  async store(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const tags: string[] = body.tags.split(',').map((t) => t.trim());
      ...
      const responseArticle = await prisma.article.findFirst({
        where: { id: article.id },
        include: {
      ...
      if (!responseArticle) {
        res.status(500).json({
          message: 'Failed to fetch created article',
          status: 500,
        });
        return;
      }
      ...
      res.status(201).json({
        ...
        data: {
          ...
          tags: responseArticle.tags.map((tag: ArticleTag & { Tag: Tag }) => ({
            ...tag.Tag,
            id: tag.Tag.id.toString(),
          })),
        },
